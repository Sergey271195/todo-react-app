...
def get_last_task_index(employee_id):
    ...
    last_index = DailyTaskConnector.objects.filter(
        employee_id__bitrix_id=employee_id, task_list__date=today
        ...
        for user in request_body:
            for index, task in enumerate(request_body[user]):
                DailyTaskConnector.objects.filter(
                    task_list=todolist, task__bitrix_id=int(task["taskId"])
...
def addExistingTask(task):
    try:
        responsible = EmployeeSerializer(
            Employee.objects.get(bitrix_id=int(task.get("responsibleId")))
        ).data
        creator = EmployeeSerializer(
            Employee.objects.get(bitrix_id=int(task.get("createdBy")))
        ).data
        new_serializer = TaskSerializer(
            data={
                "employee_id": responsible,
                "creator_id": creator,
                "bitrix_id": int(task.get("id")),
...
def removeTaskFromDailyList(task_id, todolist):
    try:
        ...
        daily_task = DailyTaskConnector.objects.get(
            task=db_task, task_list=todolist
        ).delete()
...
def addTaskToDailyList(task, todolist, employee_id, priority, comment=None, token=None):
    try:
        ...
        connector = DailyTaskConnector(
            employee_id=employee, task_list=todolist, task=task, priority=priority
        )
        connector.save()
...
def addTaskView(request, emplId):
    ...
    if request.method == "GET":
        try:
            ...
            return JsonResponse(
                users_tasks, safe=False, json_dumps_params={"ensure_ascii": False}
            )
        except Exception as e:
            return not_found_response()
        ...
        if check_bitrix_token(token):
            ...
            if emplId in [26, 334]:
                ...
                bitrix_response = tskManager.create_task(
                    employee=emplId,
                    title=request_body.get("title"),
                    groupId=request_body.get("groupId"),
                )
                task = bitrix_response["result"]["task"]
                db_task = addExistingTask(task)
            if db_task:
...
def userManagerView(request):
    ...
    if request.method == "GET":
        ...
        return JsonResponse(
            serialzer.data, safe=False, json_dumps_params={"ensure_ascii": False}
        )
...
def getListOfGroupTasks(request, pk):
    if request.method == "GET" and pk != None:
        start_date = request.GET.get("startDate")
        end_date = request.GET.get("endDate")
        bitrix = BitrixIntegrator()
        try:
            active_groups = bitrix.get_list_of_group_tasks(
                task_id=pk, start_date=start_date, end_date=end_date
            )
            return JsonResponse(
                active_groups, safe=False, json_dumps_params={"ensure_ascii": False}
            )
...
def sort_priorities(employee_id, old_priority, new_priority):
    updated_task = DailyTaskConnector.objects.get(
        employee_id__id=employee_id, priority=old_priority
    )
    if new_priority > old_priority:
        update_values = {"range": (old_priority, new_priority), "shift": -1}
    else:
        update_values = {"range": (new_priority, old_priority), "shift": 1}
    tasks_to_update = DailyTaskConnector.objects.get(
        employee_id__id=employee_id, priority__range=(update_values.get("range"))
