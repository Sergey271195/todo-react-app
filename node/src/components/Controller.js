...
const Contorller = () => {
    ...
    const prepareDailyList = (tasks) => {
        const dailyList = tasks.reduce((obj, taskObj) => {
            ...
            obj[task.fullName] =
                obj[task.fullName] === undefined
                    ? [task]
                    : [...obj[task.fullName], task];
            return obj;
        }, {});
        return dailyList;
    };
    ...
    const fetchDailyTasks = (date) => {
        ...
        fetch(`api/daily${date}`)
            ...
            .then((data) => {
                if (data.status != 404) {
                    const dailyTasksId = data.map(
                        ({ task: { bitrix_id } }) => bitrix_id
                    );
                    const dailyList = prepareDailyList(data);
                    dispatchDaily({
                        type: "GET_DAILY",
                        tasks: dailyList,
                        tasksId: dailyTasksId,
                    });
                    setError(false);
