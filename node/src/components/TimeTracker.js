...
const TimeTracker = ({ emplId, taskId, time, rightDate, employee, index }) => {
    ...
    useEffect(() => {
        ...
        if (active) {
            ...
            setTracking(
                handleTime(totalTime + Math.floor((now - start) / 1000))
            );
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (active) {
            tracker = setInterval(() => {
                setTracking(handleTime(tracking.total + 1));
            }, 1000);
        }
        return () => {
    ...
    const startTracking = () => {
        ...
        const activeArray = dailyTasks.tasks[employee].filter(
            (item) => item.time.active
        );
                ...
                <>
                    {active ? (
                        <BiPauseCircle
                            ...
                            title="Завершить учет затраченного времени (Затраченное время будет сохранено в Битрикс24)"
                        />
                    ) : (
                        <BiTimeFive
                            className="timeBtn"
                            onClick={() => startTracking()}
                            title="Начать учет затраченного времени"
                        />
                    )}
                </>
            ) : (
